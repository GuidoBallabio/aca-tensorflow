...
def fake_tfclassifier():
    def fp_fn(train_mode_placeholder):
        inputs = tf.placeholder(tf.float32, shape=[None, 2,2,2], name="features")
    ...
    def eval_fn(predictions):
        ...
        eval_metrics = {
            ...
            tf.metrics.mean_squared_error(
                labels=tf.cast(labels, tf.float32), predictions=predictions["logits"])
        }

        return eval_metrics

    return TfClassifier('name', fp_fn, loss_fn, eval_fn, tf.train.AdamOptimizer())
...
def test_split_and_batch():
    ...
    with tf.Session(graph=model.train_ops_graph[1]) as s:
        split = model._split_and_batch(inputs, input_names, batch_size, validation_split)
        input_tensors = [tf.get_default_graph().get_tensor_by_name(n + ':0') for n in input_names]
